...
// GOP Cache Implementation
    bool GOPCache::isKeyframe(const std::vector<uint8_t> &data)
    {
        ...
        return frame_type == 1; // 1 = keyframe (IDR)
    }
                                 ...
                                 uint32_t timestamp)
    {
        ...
        if (has_keyframe)
        {
            ...
            if (frames.size() > 300)
            {
                ...
                {
                    if (frames[i].type == MessageType::VIDEO && isKeyframe(frames[i].data))
                    {
    ...
    void GOPCache::sendToPlayer(RTMPSession* session)
    {
        ...
        {
            session->sendChunk(4, frame.timestamp, (uint8_t)frame.type,
...
// FLV Recorder Implementation
    ...
    void FLVRecorder::writeFLVTag(uint8_t tag_type,
                                  ...
                                  uint32_t timestamp)
    {
        if (!recording || !file.is_open()) return;
        std::lock_guard<std::mutex> lock(file_mutex);
    ...
    void FLVRecorder::writeMetadata(const
        ...
        const std::map<std::string, std::shared_ptr<AMF0Value>> &metadata)
    {
        ...
        result.push_back(0x00); result.push_back(0x00); result.push_back(0x09);
        return result;
    }
...
// RTMPSession Implementation
    ...
    {
        uint8_t c0c1[1537];
        if (!readExactly(c0c1, 1537)) return false;
        ...
        if (!writeExactly(s0s1, 1537)) return false;
        if (!writeExactly(c0c1 + 1, 1536)) return false;
        uint8_t c2[1536];
        if (!readExactly(c2, 1536)) return false;
        return true;
    }
    ...
    bool RTMPSession::parseChunkHeader(ChunkHeader& header)
    {
        ...
        {
            uint8_t buf[11];
            if (!readExactly(buf, 11)) return false;
            header.timestamp = readUint24BE(buf);
            header.msg_length = readUint24BE(buf + 3);
            header.msg_type_id = buf[6];
            header.msg_stream_id = buf[7] | (buf[8] << 8) | (buf[9] << 16) |
                                   (buf[10] << 24);
            if (header.timestamp == 0xFFFFFF)
            {
                uint8_t ext_ts[4];
                if (!readExactly(ext_ts, 4)) return false;
        ...
        {
            uint8_t buf[7];
            if (!readExactly(buf, 7)) return false;
        ...
        {
            uint8_t buf[3];
            if (!readExactly(buf, 3)) return false;
            uint32_t timestamp_delta = readUint24BE(buf);
    ...
    {
        ChunkHeader header;
        if (!parseChunkHeader(header)) return false;
        ...
        if (!readExactly(chunk_data.data(), to_read)) return false;
                                ...
                                uint32_t stream_id, const std::vector<uint8_t> &data)
    {
        ...
        while (sent < data.size())
        {
            ...
            chunk.insert(chunk.end(), data.begin() + sent, data.begin() + sent + to_send);
            if (!writeExactly(chunk.data(), chunk.size())) return false;
            sent += to_send;
            first = false;
        }
        return true;
        ...
        switch (val->type)
        {
            ...
            case AMF0Type::STRING:
                {
                    ...
                    offset += 2;
                    if (offset + str_len > len) return nullptr;
            ...
            case AMF0Type::OBJECT:
                while (offset < len)
                {
                    ...
                    if (offset + key_len > len) return nullptr;
    ...
    std::vector<uint8_t> RTMPSession::encodeAMF0Object(const
    ...
    {
        if (msg.payload.size() < 2) return true;
    ...
    bool RTMPSession::handleDataMessage(const RTMPMessage& msg)
    {
        ...
        if (!command || command->type != AMF0Type::STRING) return true;
        if (command->string == "@setDataFrame" || command->string == "onMetaData")
        {
            if (command->string == "@setDataFrame")
            {
                auto metadata_name = decodeAMF0(msg.payload.data(), msg.payload.size(), offset);
                if (!metadata_name || metadata_name->type != AMF0Type::STRING) return true;
            }
            auto metadata_obj = decodeAMF0(msg.payload.data(), msg.payload.size(), offset);
    ...
    bool RTMPSession::handleCommand(const RTMPMessage& msg)
    {
        ...
        if (args.empty() || args[0]->type != AMF0Type::STRING) return false;
    ...
    bool RTMPSession::handleConnect(const std::vector<std::shared_ptr<AMF0Value>>
                                    &args)
    {
        ...
        if (args[2]->type == AMF0Type::OBJECT)
        {
            ...
            if (props.find("app") != props.end() && props["app"]->type == AMF0Type::STRING)
            {
    ...
    bool RTMPSession::handleReleaseStream(const
                                          std::vector<std::shared_ptr<AMF0Value>> &args)
    {
        return true;
    }

    bool RTMPSession::handleFCPublish(const
                                      std::vector<std::shared_ptr<AMF0Value>> &args)
    {
        return true;
    }

    bool RTMPSession::handleCreateStream(const
                                         std::vector<std::shared_ptr<AMF0Value>> &args)
    {
    ...
    bool RTMPSession::handlePublish(const std::vector<std::shared_ptr<AMF0Value>>
                                    &args)
    {
    ...
    bool RTMPSession::handlePlay(const std::vector<std::shared_ptr<AMF0Value>>
                                 &args)
    {
        if (args.size() < 4) return false;
        if (args[3]->type == AMF0Type::STRING)
    ...
    bool RTMPSession::sendPlayResponse()
    {
        ...
        info.object["description"] = std::make_shared<AMF0Value>();
        info.object["description"]->type = AMF0Type::STRING;
        info.object["description"]->string = "Stream is now playing.";
        auto info_enc = encodeAMF0(info);
        response.insert(response.end(), info_enc.begin(), info_enc.end());
        return sendChunk(5, 0, (uint8_t)MessageType::COMMAND_AMF0,
                         stream_info.stream_id, response);
    ...
    bool RTMPSession::handleDeleteStream(const
                                         std::vector<std::shared_ptr<AMF0Value>> &args)
    {
        stream_info.is_publishing = false;
        stream_info.is_playing = false;
...
// RTMPServer Implementation
    ...
    void RTMPServer::handleClient(std::shared_ptr<RTMPSession> session)
    {
        ...
        while (running && connection_active)
        {
            ...
            if (info.is_publishing && !publish_notified)
            {
                ...
                if (auth_callback && !auth_callback(info.app, info.stream_key, info.client_ip))
                {
                    LOG_WARN("Authentication failed for: " + info.app + "/" + info.stream_key);
                ...
                {
                    LOG_WARN("Publisher limit reached for: " + info.app + "/" + info.stream_key);
            ...
            if (info.is_playing && !play_notified)
            {
                ...
                {
                    LOG_WARN("Player limit reached for: " + info.app + "/" + info.stream_key);
        ...
        while (!session->getMessageQueue().empty())
        {
            ...
            switch (type)
            {
                case MessageType::AUDIO:
                    ...
                    // Relay to players
                    sendAudioToPlayers(info.app, info.stream_key,
                                       msg.payload, msg.header.timestamp);
                    break;
                case MessageType::VIDEO:
                    if (on_video_data)
                    {
                    ...
                    // Relay to players
                    sendVideoToPlayers(info.app, info.stream_key,
                                       msg.payload, msg.header.timestamp);
                    break;
                case MessageType::DATA_AMF0:
                    {
                        size_t offset = 0;
                        auto command = session->decodeAMF0(msg.payload.data(),
                        ...
                        if (command && command->type == AMF0Type::STRING)
                        {
                            ...
                            auto metadata_obj = session->decodeAMF0(msg.payload.data(),
                                                                    msg.payload.size(), offset);
                            if (metadata_obj && (metadata_obj->type == AMF0Type::OBJECT ||
                                                 metadata_obj->type == AMF0Type::ECMA_ARRAY))
                            {
                                        ...
                                        const std::vector<uint8_t> &data, uint32_t timestamp)
    {
        ...
        for (auto& session : sessions)
        {
            ...
            if (info.is_playing && info.app == app && info.stream_key == stream_key)
            {
                session->sendChunk(4, timestamp, (uint8_t)MessageType::VIDEO,
                                   info.stream_id, data);
            }
        }
        return true;
                                           ...
                                           const std::vector<uint8_t> &data)
    {
        ...
        for (auto& session : sessions)
        {
            ...
            if (info.is_playing && info.app == app && info.stream_key == stream_key)
            {
                session->sendChunk(4, 0, (uint8_t)MessageType::DATA_AMF0,
                                   info.stream_id, data);
            }
        }
    }
                                    ...
                                    const std::string& stream_key) const
    {
        ...
        {
            const auto& info = session->getStreamInfo();
            if (info.is_publishing && info.app == app && info.stream_key == stream_key)
            {
                count++;
            }
        }
    ...
    StreamStatistics RTMPServer::getStreamStats(const std::string& app,
            const std::string& stream_key) const
    {
        std::lock_guard<std::mutex> lock(sessions_mutex);
